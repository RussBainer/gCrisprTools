...
```{r contrast interaction, eval = FALSE}
library(Biobase)
library(limma)
library(gCrisprTools)
...
#Number of replicated signals observed in the follow-up screen
# Targets changing over time in both timecourses
time.effect <- ct.compareContrasts(allSimple$ControlTime,
                                   allSimple$DeathOverTime)
summary(time.effect$replicated)

#Targets with modifying effect that compromise intrinsic viability
mod.control <- ct.compareContrasts(allSimple$Interaction,
                                 allSimple$ControlTime,
                                 same.dir = FALSE)
summary(mod.control$replicated)

#Targets with modifying effect that affect viability in death condition
mod.death <- ct.compareContrasts(allSimple$Interaction, 
                                   allSimple$ControlTime)
summary(mod.death$replicated)

#Modifiers w/ no evidence of background viability effects
