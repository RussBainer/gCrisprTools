...
##' data('fit')
##' data('ann')
##' genePvals <- ct.RRAaPvals(fit$p.value, ann, permute = 100, multicore = FALSE)
##' genePvals <- ct.RRAaPvals(fit$p.value, ann, permute = 100, multicore = TRUE, core.perm = 10)
##' @export
...
##' batch.size <- 50
##' env <- new.env()
##' env$obs <- ct.RRAalpha(fit$p.value, ann, return.obj = 'none')
##' env$ngenes <- length(levels(ann$geneSymbol))
##' passed <- ct.RRAalphaBatch(fit$p.value, ann, result.environment = env, batch.size = batch.size)
